...
func _ready():
	...
	var skin_data = Global.skins[Global.equipped_skin_id]
	sprite.texture = skin_data["idle"]
...
func _physics_process(delta):
	...
	if is_on_floor():
		if abs(velocity.x) > 10:
			anim.play("run")
			sprite.texture = current_skin["run"]
			if current_skin.has("run_frames"):
				sprite.hframes = current_skin["run_frames"]
			else:
				sprite.hframes = 5
		else:
			anim.play("idle")
			sprite.texture = current_skin["idle"]
			sprite.hframes = 5
	else:
		sprite.texture = current_skin["run"]
		if current_skin.has("run_frames"):
...
func _process(_delta):
	# Update the text to show global money
	money_label.text = "RAM DOWNLOADED: " + str(Global.money)
